...
def home():
  ...
  return template('views/home', get_url = webApp.get_url, tasks=tasks )
...
@webApp.get('/task')
def create_task():
  return template('views/task_form', get_url = webApp.get_url, task=None )

@webApp.get('/task/<taskid>')
def create_task(taskid):
  db = model.Session()
  task = db.query(model.Task).get(taskid)
  db.close()
  return template('views/task_form', get_url = webApp.get_url, task=task )

@webApp.get('/delete_task/<taskid>')
...
def save_task():
  ...
  if request.POST.get('id') is not None:
    task = db.query(model.Task).get(request.POST.get('id')) 
  
  task.name = request.POST.get('name')
  task.description = request.POST.get('description')
  task.trigger = request.POST.get('trigger')
  task.status = 'stopped'
